'''
该代码用来从 VOC格式的annotations/*.xml 生成 YOLO格式的labels/*.txt;
从xml文件中提取对应于图片的注释信息，并将其按yolo训练所需的格式存储在*.txt文件中，
一个xml文件生成一个对应的txt文件
'''
import os.path
...
xmlpath = 'D:\\Projects\\insect_yolo\\before_transform\\Annotations'  # 原xml路径
txtpath = 'D:\\Projects\\insect_yolo\\transformed\\labels'  # 转换后txt文件存放路径
files = []
